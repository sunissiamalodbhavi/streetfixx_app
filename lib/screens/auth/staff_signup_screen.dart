...
class _StaffSignupScreenState extends State<StaffSignupScreen> {
    ...
    if (_formKey.currentState!.validate()) {
      ...
      try {
        await _apiService.signup(
          ...
          rollNo: '', // Not used for staff
          gender: _selectedGender!,
          staffId: _staffIdController.text,
        );
        if (mounted) {
          ScaffoldMessenger.of(context).showSnackBar(
            const SnackBar(content: Text('Staff Signup Successful! Please Login.')),
          );
          Navigator.pop(context); // Go back to Staff Auth Screen (Login/Signup choice)
        }
      } catch (e) {
        if (mounted) {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(title: const Text('Staff Signup')),
          ...
          child: Column(
            children: [
              ...
              TextFormField(
                ...
                validator: (v) => (v == null || v.isEmpty) ? 'Staff ID is required' : null,
              ),
              const SizedBox(height: 20),
